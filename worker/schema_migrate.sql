...
CREATE TABLE IF NOT EXISTS ebay_sold_observations (id INTEGER PRIMARY KEY AUTOINCREMENT, set_num TEXT NOT NULL REFERENCES lego_sets(set_num), engine TEXT NOT NULL CHECK(engine IN ('firecrawl','apify')), observed_at TEXT NOT NULL, condition TEXT NOT NULL CHECK(condition IN ('new_sealed','used_complete','unknown')), source_url TEXT, item_id TEXT, title TEXT, price_usd REAL, sold_date TEXT, reference_value REAL, reference_provenance TEXT NOT NULL, reference_freshness TEXT NOT NULL CHECK(reference_freshness IN ('fresh','stale','missing')), decision TEXT NOT NULL CHECK(decision IN ('accepted','rejected','review_needed','no_data','error')), rejection_reason TEXT);
...
ALTER TABLE user_collection ADD COLUMN sold_fees REAL;
ALTER TABLE user_collection ADD COLUMN sell_target REAL;
ALTER TABLE user_collection ADD COLUMN sell_target_alerted_at TEXT;
...
ALTER TABLE user_prefs ADD COLUMN notify_sell_targets INTEGER;
ALTER TABLE user_prefs ADD COLUMN notify_big_moves INTEGER;
ALTER TABLE user_prefs ADD COLUMN notify_retiring INTEGER;
ALTER TABLE user_prefs ADD COLUMN notify_back_in_stock INTEGER;
ALTER TABLE user_prefs ADD COLUMN quiet_hours INTEGER DEFAULT 0;
ALTER TABLE user_prefs ADD COLUMN quiet_start INTEGER DEFAULT 22;
ALTER TABLE user_prefs ADD COLUMN quiet_end INTEGER DEFAULT 8;
ALTER TABLE user_prefs ADD COLUMN timezone TEXT;
