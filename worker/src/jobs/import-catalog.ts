  ...
  for (const s of sets) {
    ...
    setStmts.push(db.prepare(`
      ...
      ON CONFLICT (set_num) DO UPDATE SET
        ...
        retail_price=CASE WHEN lego_sets.valuation_method IN ('ai','market','ebay_rss','brickeconomy') THEN lego_sets.retail_price ELSE EXCLUDED.retail_price END,
        current_value=CASE WHEN lego_sets.valuation_method IN ('ai','market','ebay_rss','brickeconomy') THEN lego_sets.current_value ELSE EXCLUDED.current_value END,
        forecast_2y=CASE WHEN lego_sets.valuation_method IN ('ai','market','ebay_rss','brickeconomy') THEN lego_sets.forecast_2y ELSE EXCLUDED.forecast_2y END,
        forecast_5y=CASE WHEN lego_sets.valuation_method IN ('ai','market','ebay_rss','brickeconomy') THEN lego_sets.forecast_5y ELSE EXCLUDED.forecast_5y END,
        valuation_method=CASE WHEN lego_sets.valuation_method IN ('ai','market','ebay_rss','brickeconomy') THEN lego_sets.valuation_method ELSE 'formula_bulk' END,
