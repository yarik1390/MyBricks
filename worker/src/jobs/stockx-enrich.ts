...
import { fetchStockXMarket } from '../lib/stockx';
import { pickKey } from '../lib/brightdata-keys';
import { stockxEnabled } from '../lib/pricing-flags';
...
/**
 * StockX lowest-ask enrichment (Bright Data Web Unlocker, rendered).
 *
 * Populates lego_sets.stockx_ask for sets that ALREADY have a BrickLink/
 ...
 * OFF by default (stockxEnabled): a slow (~30-45s/call), fragile scrape that must
 * be validated before it feeds anything. Currently COLLECT-ONLY — stockx_ask is
 * stored for review and is NOT yet wired into the blend (a deliberate follow-up
 * once the data proves out). Bounded, negative-cached, health-recorded; never
 * throws. Runs only from the background cron — never a synchronous request.
 */
export async function runStockXEnrich(
  ...
  // A live token must exist before any selection work, so an exhausted pool
  // never debits the quota ledger for a run that makes zero calls.
  ...
  const capLimit = Math.min(limit, remaining);
  ...
  const effLimit = (await reserveQuota(env, { stockx: filtered.length })).stockx ?? 0;
  if (effLimit <= 0) return { processed: 0, updated: 0, rejected: 0, limit, skipped: 'stockx quota spent' };
  const results = filtered.slice(0, effLimit);
  ...
  for (let i = 0; i < results.length; i += concurrency) {
    ...
    const outs = await Promise.all(batch.map(async (set) => ({
      set,
      r: await fetchStockXMarket(set.set_num, set.name, env).catch((e) => ({ status: 'error' as const, ask: null, url: null, error: (e as Error)?.message })),
    })));
