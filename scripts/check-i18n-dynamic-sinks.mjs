...
const productionRegressions = [
  ...
  ['public/js/views/catalog.js', /tPlural\('catalog\.filtersWithCount', n\)/, /\$\{n\}\s+active|\$\{n\}\s+filters/],
  ...
  ['public/js/components/scanner.js', /tPlural\("bvAdd\.ownCopies", qty, \{ count: qty, next: qty \+ 1 \}\)/, /title:\s*`Already owned`|message:\s*`You already have|confirmLabel:\s*'Add anyway'|You own \$\{/],
  ['public/js/components/scanner.js', /tPlural\("scanner\.minifigsFound", minifigs\.length\)[\s\S]*?tPlural\("bvAdd\.shelfFound", sets\.length, \{ count: sets\.length \}\)/, /0 sets found|sets\.length\} SETS FOUND|sets\.length\} sets found/],
  ...
  ['public/js/components/scanner.js', /const badge = result\.kids\.new_badges\?\.\[0\] \? kidsBadgeLabel\(result\.kids\.new_badges\[0\]\) : "";[\s\S]*?kidsXpMessage\(result\.kids\.xp_gained, \{ level: result\.kids\.new_level, badge \}\)/, /kidsXpMessage\([^)]*badge: kidsBadgeLabel\(/],
