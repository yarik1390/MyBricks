...
// medians in community_comps. WRITE-ONLY for now: the blend does not read it
// until sample quality is proven (the same dual-write discipline pricing v3
// used). Privacy and integrity rules:
...
export async function runCommunityComps(env: Env) {
  ...
  for (const g of groups.values()) {
    ...
    `).bind(
      g.set_num, g.bucket,
      round2(median(sorted)), round2(percentile(sorted, 0.25)), round2(percentile(sorted, 0.75)),
  ...
  return { published: stmts.length, pruned, candidates: groups.size };
}
