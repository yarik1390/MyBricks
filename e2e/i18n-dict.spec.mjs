...
test.describe(() => {
  ...
  test('set names and numbers are never translated', async ({ page }) => {
    // Title and the monospace set number in the meta line are product data.
    ...
    await page.locator('.bv-settitle__name').waitFor();
    await page.waitForTimeout(600);
    // Product data must pass through untouched.
    await expect(page.locator('.bv-settitle__name')).toContainText('Millennium Falcon');
    await expect(page.locator('.bv-settitle__num')).toHaveText('75192-1');
  });
});
