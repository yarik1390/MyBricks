...
describe('runWishlistAlerts', () => {
  ...
  it('fires a price-drop alert and stamps the cooldown', async () => {
    await db.batch([
      db.prepare(`INSERT INTO lego_sets (set_num, name, current_value) VALUES ('D-1','Drop Set', 50)`),
  ...
  it('respects the 7-day drop cooldown', async () => {
    await db.batch([
      db.prepare(`INSERT INTO lego_sets (set_num, name, current_value) VALUES ('D-2','Recent', 50)`),
  ...
  it('fires a value-spike alert for a +30% collection gain', async () => {
    await db.batch([
      db.prepare(`INSERT INTO lego_sets (set_num, name, current_value) VALUES ('S-1','Spiker', 200)`),
