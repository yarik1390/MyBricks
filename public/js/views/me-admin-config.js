...
export const ADMIN_JOB_TOOLS = {
  ...
  pricechartingBulk: {
    ...
    desc: 'Downloads the whole LEGO price guide, updates values + liquidity, and auto-verifies unique-UPC mappings.',
    source: 'PriceCharting CSV',
    duration: 'One ~2MB download',
    quota: 'Auto-runs weekly (Sunday); CSV downloads are limited to once per 10 minutes. Needs the Legendary-tier token.',
    icon: I.download(),
  },
  pricechartingVerify: {
    url: '/api/admin/run-pricecharting-verify',
    method: 'POST',
    body: {},
    label: 'Verify PriceCharting mappings',
    desc: 'Promotes mappings whose PC price agrees with an independent BrickLink/eBay sold value, then re-blends them.',
    source: 'PriceCharting (stored data)',
    duration: 'Seconds — set-based SQL, no external calls',
