...
import { collectionRoomCatalog, displayCartonDimensions, resolveBoxBackImageUrl } from '../lib/collection-room.js';
...
export async function renderCollectionRoom() {
  ...
  function details(setNum) {
    ...
    modal(`<section class="showroom-inspect" aria-describedby="roomInspectHelp"><h2 id="roomSheetTitle">${escapeHtml(item.name)}</h2><p class="showroom-inspect-kicker">${t('room.inspecting')}</p><div class="showroom-inspect-turntable" id="roomTurntable" tabindex="0" role="img" aria-label="${escapeHtml(t('room.inspectLabel', { name: item.name }))}"><div class="showroom-inspect-box" id="roomInspectBox" style="--inspect-width:min(64vw, 320px); --inspect-height:calc(var(--inspect-width) / ${aspect.toFixed(3)}); --inspect-depth:calc(var(--inspect-width) * ${depthRatio.toFixed(3)});"><div class="showroom-inspect-face showroom-inspect-front"><img src="${imageUrl(item)}" alt="${escapeHtml(item.name)}"></div><div class="showroom-inspect-face showroom-inspect-back">${backUrl ? `<img src="${proxyImg(backUrl)}" alt="">` : `<div class="inspect-back-inner"><div class="inspect-badge-row"><span class="inspect-lego-badge">LEGO</span><span class="inspect-back-theme">${escapeHtml(themeName(item))}</span></div><div class="inspect-back-title">${escapeHtml(item.name)}</div><div class="inspect-back-specs"><div class="inspect-spec-pill"><span>SET</span><strong>${escapeHtml(item.set_num)}</strong></div>${item.pieces ? `<div class="inspect-spec-pill"><span>PIECES</span><strong>${item.pieces}</strong></div>` : ''}${item.year ? `<div class="inspect-spec-pill"><span>YEAR</span><strong>${item.year}</strong></div>` : ''}</div><div class="inspect-back-legal"><span>LEGO System A/S · DK-7190 Billund</span><span>Official Licensed Product</span></div></div>`}</div><div class="showroom-inspect-face showroom-inspect-left"><div class="inspect-spine-inner"><span class="inspect-lego-badge-mini">LEGO</span><span class="inspect-spine-num">${escapeHtml(item.set_num)}</span><span class="inspect-spine-name">${escapeHtml(item.name)}</span>${item.pieces ? `<span class="inspect-spine-pieces">${item.pieces} pcs</span>` : ''}</div></div><div class="showroom-inspect-face showroom-inspect-right"><div class="inspect-spine-inner"><span class="inspect-spine-theme">${escapeHtml(themeName(item))}</span><span class="inspect-spine-num">${escapeHtml(item.set_num)}</span><span class="inspect-spine-year">${item.year || ''}</span></div></div><div class="showroom-inspect-face showroom-inspect-top"><div class="inspect-flap-inner"><span class="inspect-lego-badge-mini">LEGO</span><span class="inspect-flap-text">${escapeHtml(item.name)} · ${escapeHtml(item.set_num)}</span>${item.pieces ? `<span class="inspect-flap-pieces">${item.pieces} pcs</span>` : ''}</div></div><div class="showroom-inspect-face showroom-inspect-bottom"><div class="inspect-bottom-inner"><div class="inspect-barcode-lines" aria-hidden="true"></div><div class="inspect-bottom-text">${escapeHtml(item.set_num)}</div></div></div></div></div><p id="roomInspectHelp" class="showroom-inspect-help">${t('room.rotateHint')}</p><div class="showroom-inspect-rotate" aria-label="${escapeHtml(t('room.rotateControls'))}"><button type="button" id="roomRotateLeft" aria-label="${escapeHtml(t('room.rotateLeft'))}">↶</button><button type="button" id="roomRotateRight" aria-label="${escapeHtml(t('room.rotateRight'))}">↷</button></div><dl class="showroom-inspect-facts"><div><dt>${t('room.setNumber')}</dt><dd>${escapeHtml(item.set_num)}</dd></div><div><dt>${t('room.theme')}</dt><dd>${escapeHtml(themeName(item))}</dd></div><div><dt>${t('room.owned')}</dt><dd>${escapeHtml(tPlural('room.copies', item.quantity, { quantity: item.quantity }))}</dd></div></dl><a class="btn-primary" id="roomFullDetails" href="#/set/${encodeURIComponent(item.set_num)}">${t('room.fullDetails')}</a></section>`, stage, { preservePickup: true });
    let rotation = 0;
